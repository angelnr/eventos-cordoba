...
function addComputedFields(events) {
...
async function addFavoriteStatus(events, userId) {
...
async function getUpcomingEvents(userId, options = {}) {
...
async function getPastEvents(userId, options = {}) {
  const pagination = parsePagination(options.query || {});
  const page = pagination.page;
  const limit = options.paginate ? pagination.limit : SUMMARY_LIMIT;
  const skip = options.paginate ? pagination.skip : 0;
...
async function getFavoriteEvents(userId, options = {}) {
  const pagination = parsePagination(options.query || {});
  const page = pagination.page;
  const limit = options.paginate ? pagination.limit : SUMMARY_LIMIT;
  const skip = options.paginate ? pagination.skip : 0;
  ...
  if (options.categoryId) {
    where.event.categoryId = parseInt(options.categoryId);
  }
...
async function getOrganizedEvents(userId, options = {}) {
  const pagination = parsePagination(options.query || {});
  const page = pagination.page;
  const limit = options.paginate ? pagination.limit : SUMMARY_LIMIT;
  const skip = options.paginate ? pagination.skip : 0;
...
async function getMyEventsSummary(userId) {
